...
def merge_conv(
    weight_a: torch.Tensor,#nn.Parameter|torch.Tensor,
    weight_b: torch.Tensor,#nn.Parameter|torch.Tensor,
    device = 'cpu'
):
...
def extract_diff(
    ...
    def make_state_dict(
        ...
        for name, module in root_module.named_modules():
            ...
            print('module class name: ', module.__class__.__name__)
            ##################################
            if module.__class__.__name__ not in {'Linear', 'Conv2d'}:#in target_replace_modules:
                continue
            temp[name] = module.weight
            print('module.weight: ', module.weight)
                # temp[name] = {}
                # for child_name, child_module in module.named_modules():
                #     print('child module name: ', child_name)
                #     print('child module class name: ', child_module.__class__.__name__)
