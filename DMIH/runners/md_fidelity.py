...
def cal_md_fidelity(ref_sample_pth='', test_sample_pth=''):
    ...
    for ref, test in zip(sorted(ref_images), sorted(test_images)):
        ...
        ref_t = torch.from_numpy(ref_img).permute(2, 0, 1).unsqueeze(0).float()
        test_t = torch.from_numpy(test_img).permute(2, 0, 1).unsqueeze(0).float()

        # 可选：如果你希望用 RGB 一致性（cv2 读出来是 BGR）
        ref_t = ref_t[:, [2, 1, 0], :, :]
        test_t = test_t[:, [2, 1, 0], :, :]

        ssim_z = ssim(test_t, ref_t, data_range=255, size_average=False)
        ssim_avg.append(ssim_z.item())
